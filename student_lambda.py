...
from LearnSoftMultiLambdaMeta_SSKD import LearnSoftMultiLambdaMeta
...
best_acc = 0
Nteacher = 1 # check
lambdas = torch.ones(args.batch_size, Nteacher+1)*(args.kd_weight/(args.kd_weight + args.ce_weight))
lambdas_ss = torch.ones(args.batch_size, Nteacher)*args.ss_weight
lambdas_t = torch.ones(args.batch_size, Nteacher)*args.tf_weight
num_cls = 100
N = 0 # dead variable
...
for epoch in range(args.epoch):
    ...
    for x, target, indices in train_loader:
        ...
        for m in range(Nteacher):
            ...
            loss += lambdas_t[indices,m]*loss4
    ...
    loss_record = AverageMeter()
    start = time.time()
    for x, target, indices in val_loader:
